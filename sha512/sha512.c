...
/*   Updated: 2018/08/09 17:02:43 by jochang          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			*sha512(char *file_name, char *str)
{
	...
	while ((m = (str ? get_str_chunk_l(str, 1) : get_chunk_l(fd, 1))))
	{
		w = sha512_w_init(m);
		hash = base;
		hash = sha512_loop(hash, w);
		base = sha512_base_set(base, hash);
		free(m);
		free(w);
	}
	if (fd != 0)
		close(fd);
	return (digest_sha512(base));
}
...
uint64_t		*digest_sha512(t_sha512_hash base)
{
	...
	digest = ft_memalloc(sizeof(uint64_t) * 8);
	...
	ft_memcpy(&digest[6], &base.g, 8);
	ft_memcpy(&digest[7], &base.h, 8);
	ft_swapnendian64(digest, 8);
	return (digest);
}
